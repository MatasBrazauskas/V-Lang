...
struct Token {
    Token() = delete;
    Token(TokenType, TokenSubType, std::string_view t_lexeme, int t_line);
...
public:
    ...
    [[nodiscard]] std::tuple<TokenType, TokenSubType> identifyAritmeticOp(std::string_view) const;
private:
...
private:
    ...
    [[nodiscard]] std::tuple<int, std::string_view> parseString(std::string_view t_input, int) const;
